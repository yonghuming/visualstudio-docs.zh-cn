---
title: "如何：使用探查器命令行检测 .NET Framework 服务和收集内存数据 | Microsoft Docs"
ms.custom: ""
ms.date: "12/15/2016"
...
manager: "ghogen"
---
# 如何：使用探查器命令行检测 .NET Framework 服务和收集内存数据
...
> [!NOTE]
>  如果计算机启动后无法重新启动某项服务（如操作系统启动时所启动的服务），则无法用检测方法分析该服务。  
>   
...
## 启动分析会话  
 若要从 [!INCLUDE[dnprdnshort](../code-quality/includes/dnprdnshort_md.md)] 服务收集性能数据，请使用 [VSPerfCLREnv.cmd](../profiling/vsperfclrenv.md) 工具初始化相应的环境变量，并使用 [VSInstr.exe](../profiling/vsinstr.md) 工具创建服务二进制文件的受检测副本。  
  
 承载该服务的计算机必须重新启动才能配置该服务进行分析。  还必须手动从服务控制管理器启动该服务。  然后启动探查器，再启动 [!INCLUDE[dnprdnshort](../code-quality/includes/dnprdnshort_md.md)] 服务。  
...
3.  使用服务控制管理器将原始二进制文件替换为受检测版本。  确保将服务的“启动类型”设置为“手动”。  
...
## 控制数据收集  
 在服务运行的同时，可以通过用 **VSPerfCmd.exe** 选项开始和停止向文件写入数据来控制数据收集。  通过控制数据收集，可以收集程序执行的特定阶段（如启动或关闭应用程序）的数据。  
...
## 结束分析会话  
 若要结束分析会话，请关闭正在运行受检测组件的应用程序，然后启动 **VSPerfCmd** [\/shutdown](../profiling/shutdown.md) 选项来关闭探查器和关闭分析数据文件。  **VSPerfClrEnv \/globaloff** 命令清除分析环境变量。  
...
3.  完成所有分析后，清除分析环境变量。  键入：  
     ...
     **VSPerfClrEnv \/globaloff**  
     ...
     使用检测模块替换原始模块。  如有必要，重新配置服务的“启动类型”。  
