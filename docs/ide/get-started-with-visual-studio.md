...
Visual Studio 是用于开发应用的强大工具。 如果尚未这么做，请下载并安装 [Visual Studio](https://aka.ms/vs/15/preview/vs_enterprise)。 有关 Visual Studio 下载及其配置方式的详细信息，请观看视频 [Visual Studio 入门 -设置 IDE](https://www.youtube.com/watch?v=xLCedknQkN0&list=PLReL099Y5nRfw6VNvzMkv0sabT2crbSpK&index=1)。
...
某些解决方案类型可能包括窗体，如 WPF 或 Windows 窗体。 在这些情况下，此位置还会显示一个可视化设计器，让你可以直观地将控件添加到窗体中，如按钮和列表框。
...
无需创建解决方案或项目即可开始编写代码。 只需在 Visual Studio 中打开代码文件（如从 Git 存储库克隆的文件）即可立即编辑它们。 这些文件会显示在“解决方案资源管理器”中，并带有语法着色和基本语句完成等，同传统解决方案一样。 有关详细信息，请参阅[打开任何文件夹](https://blogs.msdn.microsoft.com/visualstudio/2016/04/12/open-any-folder-with-visual-studio-15-preview/)。
...
请注意，工具栏图标和菜单命令可能会根据上下文发生更改，表示当前已选中该项。
...
- **安装** - 若要了解如何下载和安装 Visual Studio，请参阅[下载和安装](https://go.microsoft.com/fwlink/?linkid=833223)。
- **基础知识** - 若要深入了解 Visual Studio 的工作原理，请参阅[使用 Visual Studio 开发入门](../ide/get-started-developing-with-visual-studio.md)。
- **设置** - 自定义 Visual Studio 的工作方式。 请参阅 [Personalizing the Visual Studio IDE](https://msdn.microsoft.com/en-us/library/mt269425.aspx)（个性化 Visual Studio IDE）。
- **教程** - 若要深入了解如何在 Visual Studio 中开发代码，请浏览教程，如 [Getting Started with Visual C# and Visual Basic](https://msdn.microsoft.com/en-us/library/dd492171.aspx)（Visual C# 和 Visual Basic 入门）或 [Getting Started with C++ in Visual Studio](https://msdn.microsoft.com/en-us/library/jj620919.aspx)（Visual Studio 中的 C++ 入门）。
- **视频** - 若要深入了解 Visual Studio 的其他功能和特征，请查看 YouTube 上 [Microsoft Visual Studio 频道](https://www.youtube.com/user/VisualStudio/videos)中的视频或[第 9 频道](https://channel9.msdn.com/Tags/visual+studio)中的 Visual Studio 视频。

## <a name="access-cloud-based-resources"></a>访问基于云的资源

如果想在应用或游戏中使用基于云的资源，可以通过包含 [Azure 服务](https://azure.microsoft.com/en-us/services/)达到此目的。 可使用新的 Visual Studio 安装程序安装 Azure 工作负载来获取 Azure SDK for .NET。 安装的包与 SDK 2.9.5 版本具有相同的功能级别。 对于此版本和所有未来版本的 Visual Studio，只能通过 Visual Studio 安装程序获得 Azure SDK for .NET。

安装 Azure SDK for.NET 后，Visual Studio 中会提供称为“Cloud Explorer”的新工具窗口。 Cloud Explorer 可用于在 Visual Studio 中浏览并管理 Azure 资产和资源。 如果某一特定操作需要 Azure 门户，Cloud Explorer 会提供你在 Azure 门户中所需位置的链接。
...
安装 Azure SDK 还会提供 [Visual Studio Tools for Azure](https://www.visualstudio.com/vs/azure-tools/) 以及其他工具。
...
- [Productivity Tips for Visual Studio](https://msdn.microsoft.com/en-us/library/jj153218.aspx)（Visual Studio 的工作效率提示）
