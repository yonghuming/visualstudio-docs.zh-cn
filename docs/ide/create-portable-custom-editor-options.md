...
![EditorConfig 层次结构](~/docs/ide/media/vside_editorconfig_hierarchy.png)
...
![文本编辑器 Tab 设置](~/docs/ide/media/vside_editorconfig_tabsetting.png)

按预期，在下一行按 Tab 键会使该行首行缩进四个空格字符。

![使用 EditorConfig 之前的代码](~/docs/ide/media/vside_editorconfig_before.png)

将以下名为 .editorconfig 的新文件添加到项目。 （`[*.cs]` 设置意味着此更改仅应用于此项目中的 .cs 文件。）

![已将 .editorconfig 文件添加到项目](~/docs/ide/media/vside_editorconfig_addconfig.png)

现在，如果按 Tab 键，会获得制表符而非空格。

![使用 Tab 添加制表符](~/docs/ide/media/vside_editorconfig_tab.png)
