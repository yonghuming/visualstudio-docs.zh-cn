...
> [!NOTE]
>  由于 `CV_HREG_e` 枚举太大而无法复制全文到此处，因此提供该简化版本以供引用。  遗失项以斜体注意。  完整枚举在 cvconst.h 头文件中可用。  
...
## Elements  
 ...
 Intel 80x86 和 ix86 处理器使用的寄存器和 p 代码。  
 ...
 CV\_ALPHA\_\*  
 Digital Equipment Corporation Alpha AXP 系列处理器使用的寄存器。  
  
 CV\_PPC\_\*  
 Motorola\/IBM PowerPC 处理器使用的寄存器。  
  
 CV\_SH3\_\* 和 CV\_SH\_\*  
 Hitachi SH3 和 Hitachi 嘘系列使用的寄存器处理器。  
 ...
 CV\_TRI\_\*  
 英飞凌 TriCore 处理器使用的寄存器。  
  
 CV\_AM33\_\*  
 Matsushita\/Panasonic AM33 和相关的处理器使用的寄存器。  
