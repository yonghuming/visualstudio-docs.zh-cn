...
[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 不能实现某个事件，因为该事件的委托类型与另一个事件的委托类型不匹配。 如果在接口中定义了多个事件，然后试图用一个事件同时实现这些事件，则可能发生此错误。 只有当所有要实现的事件都使用 `As` 语法进行声明并指定相同的委托类型时，事件才能实现两个或更多个事件。  
