...
在控件返回调用方之前，自动实现的属性“name”的支持字段必须完全赋值。 请考虑从构造函数初始值设定项中调用默认构造函数。  
 ...
 [结构](/dotnet/csharp/language-reference/keywords/struct)中的造函数必须初始化该结构中的所有字段。 有关详细信息，请参阅[构造函数](/dotnet/csharp/programming-guide/classes-and-structs/constructors)。  
