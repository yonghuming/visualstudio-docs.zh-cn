...
成员“\<membername1\>”隐式声明“\<implicitmembername\>”，它与为基类“\<baseclassname\>”中的成员“\<membername2\>”隐式声明的成员冲突。 因此该成员应声明为“Shadows”。  
 ...
 派生类的成员将生成与基类的隐式成员具有相同名称的隐式成员。 由于隐式成员不指定 [Overloads](/dotnet/visual-basic/language-reference/modifiers/overloads)，编译器将假定此成员 [Shadows](/dotnet/visual-basic/language-reference/modifiers/shadows) 为隐式基类成员。 如果为此成员显式指定 `Shadows` 关键字，则你的代码将更具可读性且更不易出错。  
...
-   如果你想隐藏或遮蔽隐式基类成员，请将 [Shadows](/dotnet/visual-basic/language-reference/modifiers/shadows) 关键字包含在派生的类成员的声明中。  
