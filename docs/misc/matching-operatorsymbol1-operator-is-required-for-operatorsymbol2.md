...
-   `IsTrue` 和 `IsFalse`  
 ...
 即使没有明确地使用匹配运算符，[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 也可能需要使用它。 例如，如果定义在 [For...Next 语句](/dotnet/visual-basic/language-reference/statements/for-next-statement) 中用作计数器变量的类或结构，[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 将同时需要 `>=` 和 `<=` 运算符（以及 `+` 运算符）。  
  
 **错误 ID：**BC33033  
  
### 更正此错误  
  
-   在同一个类或结构中定义匹配运算符。 请尽量将它定义得有意义，因为 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 可能会在你未预计到的情况下使用它。  
...
## 请参阅  
 ...
 [运算符和表达式](/dotnet/visual-basic/programming-guide/language-features/operators-and-expressions/index)