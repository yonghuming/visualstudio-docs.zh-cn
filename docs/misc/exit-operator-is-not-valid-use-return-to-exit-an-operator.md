...
`Operator` 过程中出现 `Exit Operator` 语句。  
 ...
 必须使用 [Return 语句](/dotnet/visual-basic/language-reference/statements/return-statement) 从 `Operator` 过程进行返回。[Exit 语句](/dotnet/visual-basic/language-reference/statements/exit-statement) 不接受 `Operator` 关键字，`End Operator` 语句不会将控制权返回给调用代码。  
