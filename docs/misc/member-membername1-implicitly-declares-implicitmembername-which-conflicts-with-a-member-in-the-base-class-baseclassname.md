...
成员“\<membername1\>”隐式声明“\<implicitmembername\>”，它与基类“\<baseclassname\>”中的成员冲突，因此该成员不应声明为“Overloads”  
 ...
 派生类中的属性生成隐式成员（其名称与基类中某个成员的名称相同），并且指定 [Overloads](/dotnet/visual-basic/language-reference/modifiers/overloads) 关键字。  
  
 重载用于定义在同一类中属性或过程的多个版本。 除非基类成员已指定 `Overloads`，否则不能定义基类成员的其它版本。 由于冲突的基类成员不指定 `Overloads`，编译器将假定此属性 [Shadows](/dotnet/visual-basic/language-reference/modifiers/shadows) 为隐式基类成员。  
  
 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 编译器创建与你声明的某些编程元素相对应的隐式成员。 下表总结了这些隐式或*综合*成员。  
...
-   如果你想要隐藏或遮蔽基类成员，将 [Overloads](/dotnet/visual-basic/language-reference/modifiers/overloads) 关键字替换为属性的声明中的 [Shadows](/dotnet/visual-basic/language-reference/modifiers/shadows) 关键字。  
