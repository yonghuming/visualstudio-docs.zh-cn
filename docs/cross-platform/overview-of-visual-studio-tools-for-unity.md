...
本部分将介绍更多有关 Visual Studio Tools for Unity 提供的功能的内容以及如何使用它们提高 Unity 的使用效率。  
 ...
 借助 Visual Studio Tools for Unity \(*VSTU*\)，可以使用 Visual Studio 在 C\# 中编写游戏和编辑器脚本，随后使用其功能强大的调试器查找和修复错误。  VSTU 的最新版本包括用于 Unity 的 ShaderLab 着色器语言的语法着色、更好的调试器可视化效果以及针对 MonoBehavior 向导的改进的代码生成。  VSTU 还提供 Unity 项目文件、控制台消息以及在 Visual studio 中启动游戏的功能，从而使你可以在编写代码时花费更少的时间与 Unity 编辑器进行切换。  
...
-   “Unity 项目资源管理器”使用在 Unity 编辑器中显示的相同的层次结构在 Visual Studio 中显示整个 Unity 项目。  
...
-   从 Visual Studio 访问 Unity 文档。  仅突出显示你想要了解的 API 调用，然后按  CTRL\+ALT\+M、CTRL\+H.  
