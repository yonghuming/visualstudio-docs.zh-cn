...
dev_langs: 
  ...
  - "C++"
helpviewer_keywords: 
  - "部署应用程序 [ClickOnce], 安全性"
  - "ClickOnce 应用程序, caspol"
  - "安全性 [ClickOnce], WPF 浏览器承载的应用程序"
  - "安全性 [ClickOnce], ClickOnce 应用程序"
  - "ClickOnce 应用程序, 代码访问安全性策略"
  - "安全性, ClickOnce"
...
-   当应用程序需要完全信任权限时，可能会提示最终用户向其授予对该应用程序的权限。 这意味着应用程序不会真正提供 ClickOnce 体验，并且该提示可能会给经验较少的用户造成混淆。  
    ...
    > [!NOTE]
    >  当从可移动媒体（如 CD\-ROM）安装应用程序时，将不会提示用户。 此外，网络管理员可以配置网络策略，以便在从受信任的源安装应用程序时不会提示用户。 有关更多信息，请参见[受信任的应用程序部署概述](../deployment/trusted-application-deployment-overview.md)。  
