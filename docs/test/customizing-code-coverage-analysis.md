...
默认情况下，Visual Studio 代码覆盖率工具将分析单元测试过程中加载的所有解决方案程序集 \(.exe\/.dll\)。  建议保留此默认设置，因此它在大多数时间很有用。  有关详细信息，请参阅[使用代码覆盖率确定所测试的代码量](../test/using-code-coverage-to-determine-how-much-code-is-being-tested.md)。  
  
 在自定义代码覆盖率行为之前，请考虑某些替代项：  
  
-   *我需要从代码覆盖率结果中排除测试代码并只包括应用程序代码。*  
...
-   *我想包括不是我的解决方案一部分的程序集。*  
 ...
 若要自定义代码覆盖率行为，请复制[本主题结尾的示例](#sample)并使用文件扩展名 .runsettings 将其添加到你的解决方案中。  根据你自己的需要对该示例进行编辑，然后在**“测试”**菜单上，选择**“测试设置”**，再选择**“选择测试设置”**文件。  本主题的其余部分更详细地介绍了此过程。  
  
## .runsettings 文件  
 高级代码覆盖率设置在 .runsettings 文件中指定。  这是由单元测试工具使用的配置文件。  建议你复制[本主题结尾的示例](#sample)并对其进行编辑以满足你自己的需求。  
...
1.  将 .xml 文件作为扩展名为 `.runsettings` 的解决方案项添加：  
     ...
     在“解决方案资源管理器”的解决方案快捷菜单上，依次选择**“添加”**、**“新建项”**和**“XML 文件”**。  保存带有类似 `CodeCoverage.runsettings` 的名称结尾的文件  
  
2.  添加本主题结尾的示例中给定的内容，然后按你的需要自定义它，如以下章节所述。  
  
3.  在**“测试”**菜单上，选择**“测试设置”**\>**“选择测试设置文件”**，然后选择该文件。  
  
4.  现在，当运行**“分析代码覆盖率”**时，此 `.runsettings` 文件将控制其行为。  不要忘记你必须再次运行代码覆盖率：在运行测试或更新代码时，你之前的覆盖率结果和代码着色不会自动更新。  
  
5.  若要关闭和打开自定义设置，请在**“测试”**\>**“测试设置”**菜单中取消选择或选择该文件。  
  
 ![包含自定义设置文件的测试设置菜单](../test/media/codecoverage-settingsfile.png "CodeCoverage\-settingsFile")  
...
```  
 ...
 如果 `<Include>` 为空，则代码覆盖率处理会包括加载的所有程序集（.dll 和 .exe 文件）以及能为其找到 **.pdb** 文件的所有程序集，与 `<Exclude>` 列表中的子句匹配的项除外。  
...
## 如何在运行测试时指定 .runsettings 文件  
  
### 在 Visual Studio 测试中自定义 runsettings  
 依次选择**“测试”**、**“测试设置”**、**“选择测试设置文件”**和 .runsettings 文件。  该文件将显示在“测试设置”菜单上，你可以选择或取消它。  选择后，每当你使用**“分析代码覆盖率”**时都会应用 .runsettings 文件。  
  
### 在命令行测试中自定义运行设置  
 若要从命令行运行测试，请使用 vstest.console.exe。  此设置文件是此实用工具的一个参数。  有关详细信息，请参阅 [从命令行使用 VSTest.console](/devops-test-docs/test/using-vstest-console-from-the-command-line)。  
  
1.  启动 Visual Studio 开发人员命令提示符：  
  
     在 Windows**“开始”**上，依次选择**“所有程序”**、**“Microsoft Visual Studio”**、**“Visual Studio Tools”**和**“开发人员命令提示”**。  
...
### 在生成定义中自定义运行设置  
 ...
 ![在生成定义中指定 runsetting](../test/media/codecoverage-buildrunsettings.png "CodeCoverage\-buildRunsettings")  
  
1.  确保签入你的 .runsettings 文件。  
  
2.  在“团队资源管理器”中，打开**“生成”**，然后添加或编辑生成定义。  
  
3.  在**“过程”**页中，展开**“自动测试”**、**“测试源”**和**“运行设置”**。  选择 **.runsettings** 文件。  
  
    -   *但是，将显示**“测试程序集”**而不是**“测试源”**。  在尝试设置**“运行设置”**字段时，我只能选择 .testsettings 文件。*  
  
         在**“自动测试”**下，选择**“测试程序集”**，然后选择行尾的**“\[...\]”**。  在**“添加\/编辑测试运行”**对话框中，将**“测试运行程序”**设置为**“Visual Studio 测试运行程序”**。  
