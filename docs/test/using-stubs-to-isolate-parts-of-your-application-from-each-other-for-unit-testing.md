---
title: "使用存根针对单元测试隔离应用程序的各个部分 | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
*存根类型* 是 Microsoft Fakes 框架提供的让你方便地将所测试的组件与它调用的其他组件隔离的两种技术之一。  存根是一小段代码，用来在测试期间替代另一个组件。  使用存根的好处在于它能返回一致的结果，使测试更易于编写。  而且，即使在其他组件无法工作的情况下，你也可以运行这些测试。  
  
 有关 Fakes 的概述和快速入门指南，请参阅[用 Microsoft Fakes 隔离测试代码](../test/isolating-code-under-test-with-microsoft-fakes.md)。  
 ...
 由于存根依赖于你以这种方式构建代码的能力，因此你通常使用存根隔离应用程序的各个部分。  若要将它与不受你控制的其他程序集（如 System.dll）隔离开，你通常应使用填充码。  请参阅[使用填充码针对单元测试将应用程序与程序集隔离](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md)。  
...
-   [不同类型类型成员的存根](../test/using-stubs-to-isolate-parts-of-your-application-from-each-other-for-unit-testing.md#BKMK_Stub_basics)  
    ...
    -   [虚拟类的存根](../test/using-stubs-to-isolate-parts-of-your-application-from-each-other-for-unit-testing.md#BKMK_Partial_stubs)  
...
###  <a name="Dependency"></a> 依赖关系注入设计  
 若要使用存根，则必须将应用程序设计为使不同的组件不相互依赖，而只依赖接口定义。  各个组件在运行时连接在一起，而不是在编译时进行耦合。  这种模式可帮助制作可靠且易于更新的软件，因为更改往往不会跨组件边界传播。  我们建议，即使你未使用存根，也要采用此模式。  如果要编写新代码，采用[依赖关系注入](http://en.wikipedia.org/wiki/Dependency_injection)模式会很容易。  如果要为现有软件编写测试，你可能必须重构它。  如果这样做可行，则可以考虑改用填充码。  
...
##  <a name="BKMK_Stub_basics"></a> 不同类型类型成员的存根  
...
###  <a name="BKMK_Partial_stubs"></a> 虚拟类的存根  
...
2.  无法对密封类或静态方法进行存根处理，因为存根类型依赖于虚方法调度。  对于这种情况，按[使用填充码针对单元测试将应用程序与程序集隔离](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md)中所述使用填充码类型  
