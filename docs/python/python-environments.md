...
1. 在解决方案资源管理器中右键单击某项目的“Python 环境”，选择“查看所有 Python 环境”：
    ...
    ![解决方案资源管理器中的“查看所有环境”命令](~/docs/python/media/environments-view-all.png)
    
在任一情况下，Python 环境窗口都显示为解决方案资源管理器的同级选项卡：

![“Python 环境”窗口](~/docs/python/media/environments-default-view.png)
...
>
> ![“Python 环境”窗口扩展后的视图](~/docs/python/media/environments-expanded-view.png)
...
1. 在环境窗口中选择“+ 自定义...”，这将创建一个新环境，然后打开[“配置”选项卡](#configure-tab)（如下所示）。
    ...
    ![新自定义环境的默认视图](~/docs/python/media/environments-custom-1.png)
...
![Python 环境概述选项卡](~/docs/python/media/environments-overview-tab.png)
...
![Python 环境配置选项卡](~/docs/python/media/environments-configure-tab.png)
...
![Python 环境 pip 选项卡](~/docs/python/media/environments-pip-tab.png)

### <a name="intellisense-tab"></a>IntelliSense 选项卡

显示 IntelliSense 完成数据库的当前状态：

![Python 环境 IntelliSense 选项卡](~/docs/python/media/environments-intellisense-tab.png)
...
![“添加/删除 Python 环境”对话框](~/docs/python/media/environments-add-remove.png)

在解决方案资源管理器中，还可以展开环境以显示其已安装的包（该环境处于活动状态时，可以导入并在代码中使用这些包）：

![解决方案资源管理器中某环境的 Python 包](~/docs/python/media/environments-installed-packages.png)
...
1. 在解决方案资源管理器中右键单击“Python 环境”并选择“添加虚拟环境...”，这将打开以下对话框：
    ...
    ![创建虚拟环境](~/docs/python/media/environments-add-virtual-1.png)
...
1. 选择一个全局环境作为基础解释器，并单击“创建”。 如果 `pip` 和 `virtualenv`/`venv` 包不可用，将下载并安装它们。
    ...
    ![添加现有的虚拟环境](~/docs/python/media/environments-add-virtual-2.png)
...
- 如果已加载包含 `requirements.txt` 的项目，且想要安装该文件列出的所有包，请右键单击该项目并选择“从 requirements.txt 安装”：
    ...
    ![从 requirements.txt 安装](~/docs/python/media/environments-requirements-txt-install.png)

- 项目中安装有所有必要的包时，可以在解决方案资源管理器中右键单击该项目，并选择“生成 requirements.txt”来创建必要的文件。 如果文件已存在，系统将提示如何更新它：

    ![更新 requirements.txt 选项](~/docs/python/media/environments-requirements-txt-replace.png)
...
![使用 requirements.txt 创建虚拟环境](~/docs/python/media/environments-requirements-txt.png)
