...
<a name="introduction"></a> 本演练提供了 Windows Presentation Foundation \(WPF\) 开发的简介。 你将创建一个包括大多数 WPF 桌面应用程序中的常见元素的基本应用程序：XAML 标记、代码隐藏、应用程序定义、控件、布局、数据绑定和样式。  
...
4.  在“名称”文本框中输入 `ExpenseIt`，然后选择“确定”按钮。  
...
2.  在“添加新项”对话框中，选择“名称”文本框并输入 `ExpenseItHome`，然后选择“添加”按钮。  
...
2.  在“添加新项”对话框中，选择“名称”文本框并输入 `ExpenseReportPage`，然后选择“添加”按钮。  
...
4.  将现有 `<ListBox>` 元素替换为以下 XAML。  
    ...
    ```  
     ...
     此代码将 `ListBox` 的 `ItemsSource` 属性绑定到数据源并应用数据模板作为 `ItemTemplate`。  
...
## 请参阅  
 [演练：创建连接到 Azure 移动服务的 WPF 桌面应用程序](../Topic/Walkthrough:%20Create%20a%20WPF%20Desktop%20Application%20connected%20to%20an%20Azure%20Mobile%20Service.md)   
