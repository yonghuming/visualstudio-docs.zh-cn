---
title: "Office 解决方案的特定安全注意事项 | Microsoft Docs"
ms.custom: ""
ms.date: "02/02/2017"
...
dev_langs: 
  ...
  - "CSharp"
helpviewer_keywords: 
  - "Visual Studio 中的 Office 开发的疑难解答，安全性"
  - "受信任的代码 [Visual Studio 中的 Office 开发]"
  - "被阻止的代码 [Visual Studio 中的 Office 开发]"
  - "Outlook [Visual Studio 中的 Office 开发]，对象模型防护"
  - "恶意代码 [Visual Studio 中的 Office 开发]"
  - "Outlook 对象模型防护 [Visual Studio 中的 Office 开发]"
...
### 最小化对象模型防护警告  
 若要帮助在你使用受限的属性和方法时避免出现安全警告，请确保 VSTO 外接程序从项目中 `ThisAddIn` 类的 `Application` 字段获取 Outlook 对象。 有关此字段的详细信息，请参阅[VSTO 外接程序编程](../vsto/programming-vsto-add-ins.md)。  
 ...
 如果启用了对象模型防护，下面的代码示例将显示一条安全警告。Microsoft.Office.Interop.Outlook.MailItem 类的 To 属性受对象模型防护限制。Microsoft.Office.Interop.Outlook.MailItem 对象不受信任，因为代码从使用 **new** 运算符创建的 Microsoft.Office.Interop.Outlook.Application 获取它，而不是从 `Application` 字段获取它。  
 ...
 下面的代码示例演示如何使用受对象模型防护信任的 Microsoft.Office.Interop.Outlook.MailItem 对象的受限 To 属性。 该代码使用受信任的 `Application` 字段获取 Microsoft.Office.Interop.Outlook.MailItem。  
...
> [!NOTE]  
>  如果 Outlook 与 Exchange 一起使用，则从 `ThisAddIn.Application` 获取所有 Outlook 对象不能保证 VSTO 外接程序能够访问整个 Outlook 对象模型。 例如，如果 Exchange 管理员将 Outlook 设置为自动拒绝所有使用 Outlook 对象模型访问地址信息的尝试，则 Outlook 将不允许前面的代码示例访问 To 属性，即使该代码示例使用受信任的 `ThisAddIn.Application` 字段。  
  
### 指定使用 Exchange 时要信任的外接程序  
...
## Microsoft Office System 中的“信任中心”设置不影响外接程序或文档级自定义项  
...
#### 在 Microsoft Office 2010 和 Microsoft [!INCLUDE[Office_15_short](../vsto/includes/office-15-short-md.md)] 应用程序中禁用 VSTO 外接程序  
