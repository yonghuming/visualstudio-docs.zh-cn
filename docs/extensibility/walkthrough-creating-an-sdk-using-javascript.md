...
---
# 演练: 创建使用 JavaScript SDK
[!INCLUDE[vs2017banner](../code-quality/includes/vs2017banner.md)]

本演练介绍了如何使用 JavaScript 来创建一个简单的数学 SDK 作为 Visual Studio 扩展 \(VSIX\)。  本演练分为下列部分:  
...
-   [若要创建的示例应用程序使用 SDK](../extensibility/walkthrough-creating-an-sdk-using-javascript.md#createSampleApp)  
 ...
 对于 JavaScript 中，没有任何类库项目类型。 在本演练中，在 VSIX 项目中直接创建示例 arithmetic.js 文件。 在实践中，我们建议您首先生成和测试作为 Windows 应用商店应用的 JavaScript 和 CSS 文件 — 例如，通过使用 **空白应用程序** 模板 — 将其放在一个 VSIX 项目之前。  
  
## 系统必备  
 若要执行本演练中，必须安装 Visual Studio SDK。 有关详细信息，请参阅[Visual Studio SDK](../extensibility/visual-studio-sdk.md)。  
...
3.  在 **名称** 文字框中，指定 `SimpleMathVSIX` ，然后选择 **确定** 按钮。  
  
4.  如果 **Visual Studio 程序包向导** 出现，请选择 **下一步** 按钮 **欢迎** 页上，然后在 **1 页，共 7**, ，选择 **完成** 按钮。  
  
     尽管 **清单设计器** 打开时，我们将在本演练中简单通过直接修改清单的文件。  
  
5.  在 **解决方案资源管理器**, ，打开 source.extension.vsixmanifest 文件的快捷菜单，然后选择 **查看代码**。 使用此代码替换文件中的现有内容。  
...
8.  在 **解决方案资源管理器**, ，打开 SDKManifest.xml 文件的快捷菜单，然后选择 **打开** 以显示中的文件 **XML 编辑器**。  
...
11. 在 **属性** 窗口中，设置 **包含在 VSIX** 属性设置为 **True**。  
  
12. 在 **解决方案资源管理器**, ，在 SimpleMathVSIX 项目的快捷菜单，选择 **添加**, ，**新文件夹**, ，然后将该文件夹和 `Redist`。  
  
13. 添加下面要创建此文件夹结构的 Redist 子文件夹:  
  
     \\Redist\\CommonConfiguration\\Neutral\\SimpleMath\\js\\  
  
14. 在 \\js\\ 文件夹的快捷菜单，选择 **添加**, ，**新项**。  
  
15. 在下 **Visual C\# 项**, ，选择 **Web** 类别，，然后选择 **JavaScript 文件** 项。 命名该文件 `arithmetic.js`, ，然后选择 **添加** 按钮。  
  
16. 下面的代码插入到 arithmetic.js:  
...
19. 生成该项目的快捷菜单上成功完成后，选择 **在文件资源管理器中打开文件夹**。 导航到 \\bin\\debug\\，然后运行 `SimpleMathVSIX.vsix` 安装它。  
...
2.  在模板类别列表中下 **JavaScript**, ，选择 **Windows 应用商店**, ，然后选择 **空白应用程序** 模板。  
  
3.  在 **名称** 框中，指定 `ArithmeticUI`。 选择“确定”按钮。  
  
4.  在 **解决方案资源管理器**, ，打开 ArithmeticUI 项目的快捷菜单，然后选择 **添加**, ，**引用**。  
  
5.  在下 **Windows**, ，选择 **扩展**, ，您会发现， **简单的数学** 显示。  
  
6.  选择 **简单的数学** 复选框，然后选择 **确定** 按钮。  
  
7.  在 **解决方案资源管理器**, 下 **引用**, ，请注意， **简单的数学** 显示引用。 展开它，请注意，没有包括 arithmetic.js \\js\\ 文件夹。 您可以打开 arithmetic.js 若要确认已安装您的源代码。  
...
9. 使用下一步代码来替换 \\js\\default.js 的内容。  
...
11. 选择 F5 键以生成并运行该应用程序。  
  
12. 在应用程序 UI 中，输入任何两个数字，选择的操作，然后选择 **\=** 按钮。 将显示正确的结果。  
