...
|||  
|-|-|  
|类型名|UriReturnValuesShouldNotBeStrings|  
|CheckId|CA1055|  
|类别|Microsoft.Design|  
|是否重大更改|是|  
  
## 原因  
 某方法的名称包含“uri”、“Uri”、“urn”、“Urn”、“url”或“Url”，且该方法返回字符串。  
  
## 规则说明  
 该规则根据 Pascal 大小写约定将方法名称拆分为多个标记，然后检查每个标记是否与“uri”、“Uri”、“urn”、“Urn”、“url”或“Url”相等。  如果存在匹配项，则该规则假定该方法会返回统一资源标识符 \(URI\)。  URI 的字符串表示形式容易导致分析和编码错误，并且可造成安全漏洞。  <xref:System.Uri?displayProperty=fullName> 类以一种安全的方式提供这些服务。  
  
## 如何解决冲突  
 要修复与该规则的冲突，请将返回类型更改为 <xref:System.Uri>。  
  
## 何时禁止显示警告  
 如果返回值不代表 URI，则可以安全地禁止显示此规则发出的警告。  
