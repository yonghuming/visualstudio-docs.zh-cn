...
本[演练](~/extending-visual-studio-mac-walkthrough.md)主题阐释了如何生成以下扩展包，即使用“命令”将日期和时间插入到打开的文本文档中。
...
扩展/扩展点体系结构保证了快速且模块化的 Visual Studio for Mac 开发。 由于扩展类型数量繁多，本文将侧重于在[演练](~/extending-visual-studio-mac-walkthrough.md)中生成的扩展包中使用的扩展类型。

### <a name="command-extensions"></a>命令扩展

[演练](~/extending-visual-studio-mac-walkthrough.md)使用了命令扩展 - 一个指向每次执行都会调用的方法的扩展。 可通过将条目添加到 `/MonoDevelop/Ide/Commands` 扩展点来定义命令扩展。 我们使用以下代码在 `Manifest.addin.xml` 中定义了扩展：
...
为从“编辑菜单”中调用命令，该演练创建了一个插入 `/MonoDevelop/Ide/MainMenu/Edit` 扩展点的命令项扩展：
...
[演练](~/extending-visual-studio-mac-walkthrough.md)中详细介绍的扩展包与 Visual Studio for Mac 中的文本编辑器相关，但这只是其中一个自定义区域，还有很多可能的区域。 若要了解有关可用于开发的区域范围，请参阅 [Extension Tree Reference](http://monodevelop.com/Developers/Articles/Extension_Tree_Reference)（扩展树引用）和 [API Overview](http://monodevelop.com/Developers/Articles/API_Overview)（API 概述）。 生成高级扩展包时，另请参阅[开发者文章](http://monodevelop.com/Developers/Articles)。 以下是自定义区域的部分列表：
...
> [!NOTE]
我们正在致力于改善针对 Visual Studio for Mac 的扩展性方案。 如果创建了扩展，且需要其他帮助或信息，或希望提供反馈，请联系 mhutch@microsoft.com
